...
dash.register_page(__name__, name='2-Análisis de Estacionariedad', title='SARIMA | 2-Stationarity')
...
layout = dbc.Container([
    # title
    dbc.Row([
        dbc.Col([html.H3(['Análisis de Estacionariedad'])], width=12, className='row-titles')
    ]),
    # Transformations
    dbc.Row([
        dbc.Col([], width = 2),
        dbc.Col([
            dcc.Checklist(['1) Aplicar log'], persistence=True, persistence_type='session', id='log-check')
        ], width = 2),
        dbc.Col([
            dcc.Checklist(['2) Aplicar diferenciación'], persistence=True, persistence_type='session', id='d1-check'),
            dcc.Dropdown(options=[], value='', clearable=True, disabled=True, searchable=True, placeholder='Elegir lag', persistence=True, persistence_type='session', id='d1-dropdown')
        ], width = 2),
        dbc.Col([
            dcc.Checklist(['3) Aplicar diferenciación'], persistence=True, persistence_type='session', id='d2-check'),
            dcc.Dropdown(options=[], value='', clearable=True, disabled=True, placeholder='Elegir lag', persistence=True, persistence_type='session', id='d2-dropdown')
        ], width = 2),
        dbc.Col([
            dcc.Checklist(['4) Aplicar diferenciación'], persistence=True, persistence_type='session', id='d3-check'),
            dcc.Dropdown(options=[], value='', clearable=True, disabled=True, placeholder='Elegir lag', persistence=True, persistence_type='session', id='d3-dropdown')
        ], width = 2),
        dbc.Col([], width = 2),
    ...
    dbc.Row([
        dbc.Col([], width = 3),
        dbc.Col([html.P(['Test de Dickey-Fuller Aumentado: '], className='par')], width = 2),
        dbc.Col([
...
def data_transform(_logtr, _d1check, _d1v, _d2check, _d2v, _d3check, _d3v):
    ...
    if pv <= .05: # p-value
        #Stationary
        _test_output = dbc.Alert(children=['Test p-value: {:.4f}'.format(pv),html.Br(),'Los datos son ',html.B(['estacionarios'], className='alert-bold')], color='success')
    else:
        _test_output =dbc.Alert(children=['Test p-value: {:.4f}'.format(pv),html.Br(),'Los datos son ',html.B(['no estacionarios'], className='alert-bold')], color='danger')
    # Charts
    ...
    fig_1.update_layout(title='Gráfico de líneas de datos transformados', xaxis_title='Time', yaxis_title='Values')
    ...
    fig_2.update_layout(title='Gráfico de Box-Cox', xaxis_title='Promedio móvil', yaxis_title='Desviación estándar móvil')
