...
dash.register_page(__name__, name='4-Predicción', title='SARIMA | 4-Prediction')
...
layout = dbc.Container([
    # title
    dbc.Row([
        dbc.Col([html.H3(['Modelo final: Ajuste y predicción'])], width=12, className='row-titles')
    ]),
    # final-model
    dbc.Row([
        dbc.Col(width=3),
        dbc.Col([html.P(['Parámetros finales del modelo SARIMA(p,d,q)(P,D,Q)m: '], className='par')], width=6),
        dbc.Col(width=3)
    ]),
    dbc.Row([
        dbc.Col(width=3),
    ...
    dbc.Row([
        dbc.Col(width=2),
        dbc.Col([html.P(['Generar pronósticos para: '], className='par')], width=4),
        ...
        dbc.Col([dcc.Dropdown(options=['puntos de datos','años','meses','días','horas'], placeholder='puntos de datos', clearable=False, searchable=True, persistence=True, persistence_type='memory', id='n-timetype')], width=2),
        dbc.Col(width=3),
    ]),
    dbc.Row([
        dbc.Col([], width = 1),
...
def init_(_v):
    ...
    if str(_data.dtypes[0]) in (['<M8[ns]','>M8[ns]','datetime64[ns]']):
        return ['años','meses','días','horas'], 'meses'
    else:
        return ['puntos de datos'], 'puntos de datos'
...
def predict_(_p, _d, _q, _P, _D, _Q, _m, _noffs, _ntype):
    ...
    fig1.add_trace(go.Scatter(x=_data_all.loc[_data_all['_is_train']==1, 'Time'],
                                y=_data_all.loc[_data_all['_is_train']==1, 'Values'], mode='lines', name='Entrenamiento', line=train_linelayout))
    fig1.add_trace(go.Scatter(x=_data_all.loc[_data_all['_is_train']==0, 'Time'],
                                y=_data_all.loc[_data_all['_is_train']==0, 'Values'], mode='lines', name='Validación', line=test_linelayout))
    fig1.add_trace(go.Scatter(x=_data_all['Time'], y=_data_all['Values Predicted'], mode='lines', name='Predicciones', line=pred_linelayout))
    fig1.update_xaxes(title_text = 'Time')
    fig1.update_yaxes(title_text = 'Values')
    fig1.update_layout(title="Resultados del modelo final", height=500)
    ...
    fig_2.update_layout(title="Residuos del modelo: Autocorrelación (ACF)")
    fig_3.update_layout(title="Residuos del modelo: Autocorrelación Parcial (PACF)")
